#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────────────
# build-dynamo-image.sh — Build & push a custom Dynamo vLLM-runtime image from
# the RL-Scaling branch of the dynamo fork.
#
...
#   PUSH=true \
#   ./build-dynamo-image.sh
#
# Prereqs: docker, python3 (with pyyaml + jinja2 for render.py), git.
# ─────────────────────────────────────────────────────────────────────────────
set -euo pipefail

DYNAMO_DIR="${DYNAMO_DIR:-${HOME}/projects/dynamo}"
...
[[ -d "${DYNAMO_DIR}" ]] || { echo "DYNAMO_DIR not found: ${DYNAMO_DIR}"; exit 1; }
