...
| 5 | Data analysis | Three results + one honest non-claim | 2:00 | 7:15 |
...
### Result 2 — the switch improves prefill queueing (0:30)

"Within the same suite, decode→prefill improves phase-A time-to-first-token by
**31% to 39%** at both p50 and p95, versus the equal-topology control. That is the
queue-timing benefit measured directly from the frontend's own timing fields."

### Result 3 — consolidation is the strongest result (0:35)

"In the tail phase, consolidation cuts decode-GPU-seconds by **44.5%**, paired
t = −103.9. The mechanism is visible in the occupancy trace: the control holds two
...
under a second; in-flight consolidation directly removes tail GPU waste — 44.5% in
the tail phase, with a t-statistic around 100. **Limits:** this is a single node
