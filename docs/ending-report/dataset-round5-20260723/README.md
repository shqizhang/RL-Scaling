...
这组数据直接验证了新增的第四条顺序约束的设计意图:

- **相位边界切换,无交接在飞** → 出站 KV 排空**首轮轮询即通过,9–19 ms**
  (日志:`outbound KV drained (polls=1, waited=0.019s)`),成本可忽略;
- **mixed 场景下 S3 正在并发迁移 KV** → 排空等待跑满 8 s 上界后才强制过期孤儿,
  切换总耗时 9.1 s——**但该 run 仍是 100% valid、0 超时**。

也就是说:**零丢的代价不是一个常数,而是"切换时刻是否有 KV 交接在飞"的函数。**
这比"切换耗时 X 毫秒"更有信息量,也解释了为什么套件 5 会挂死 34 个请求
(那时根本没有这个等待)。
...
同样不自行调预算。事实是:该次切换与 S3 迁移并发,零丢等待跑满 8 s 上界,
**代价换来了 100% valid**。可选项:(a) 如实报告并把 G4 预算按"是否有并发交接"
分档;(b) 缩短 8 s 上界(会削弱零丢保证);(c) 让 S2/S3 在 mixed 中互斥。
我建议 (a) ——这是真实的工程权衡,藏起来反而削弱报告。
