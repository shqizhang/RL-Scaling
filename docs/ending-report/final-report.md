...
- **I1 — Abstract over-claims relative to what was measured.** The mid-report
  ...
  final abstract now reports the actual efficacy numbers — topology makespan −21.2%,
  S3 tail-phase decode-GPU·s −44.5%, mixed 100% valid — and clearly separates
  "proven strongly" from "not resolvable at this scale".
...
### 0.3 Figure suggestions (task 2)

- **Keep** the six conceptual figures (GPU-hour pattern, Dynamo architecture,
  RL-controller, K8s deployment, role-switch state machine, migration sequence).
- **F7 (add) — Phased workload timeline.** A schematic of the 59-request workload
  showing the three phases and their launch offsets (A at t0, B at +22 s, C at
  +40/43/46 s). This makes the experimental design legible at a glance.
- **F8 (add) — Per-phase decode-GPU occupancy.** Grouped bars of average decode-GPU
  count per phase for static / s3 / mixed. This is the figure that visually carries
  the −44.5% headline: the `C_tail` bar drops from 2.0 to 1.0 for S3.
- **F9 (add) — Makespan bar chart.** `T_batch` for baseline / static / s2 / s3 /
  mixed, with the −21.2% topology gap annotated.
- **Correct the migration figure caption** to note the connector path is the fast
  path *when KVBM is exposed*; recompute is the fallback otherwise.

The three data figures are described precisely enough in Part 6 that they can be
rendered from the released dataset; the `.lex` file includes their `\includegraphics`
...
- **(C5)** A rigorous, phase-attributed end-to-end evaluation on a Kubernetes
  deployment of `Qwen3-0.6B` demonstrating **correctness (100% valid across all
  scenarios)** and **efficacy** — topology makespan −21.2%, in-flight consolidation
  tail-phase decode-GPU·s −44.5% — with an honest account of what could not be
...
**Queue-timing benefit (the report-indicator requirement).** The final harness promotes
the frontend's `nvext` timings into every request row, so the switch's effect on
prefill-burst queueing is measured directly rather than inferred from wall clock.
Comparing `s2_only` against `static_2p2d` *within the same suite*, phase-A time-to-first
-token improves consistently:

| Suite | `static_2p2d` TTFT p50 / p95 | `s2_only` TTFT p50 / p95 |
|---|---|---|
| 20260722-234217 | 1222 / 2575 ms | **837 / 1761 ms** (−31.5% / −31.6%) |
| 20260723-003926 | 1410 / 2793 ms | **856 / 1724 ms** (−39.3% / −38.3%) |

**What S2 does not claim.** Phase-A *wall clock* does not improve, and the reason is
measured rather than assumed: A-phase per-request server time stays ~33–35 s across
suites regardless of the A cohort's `max_tokens` (1, 8–16, or 64–128), while TTFT is
only 0.8–1.4 s. The remaining 30+ s is the request waiting on the decode side for its
KV to arrive — this single-node cluster has no RDMA, and KV transport, not prefill
compute, bounds phase-A makespan. D→P therefore improves exactly what it can improve
(prefill queueing/TTFT) and cannot improve what the fabric bounds. Reporting a wall-clock
gain here would be attributing a transport limit to a scheduling mechanism.

**In-flight consolidation (S3), clean — the strongest, cleanest result.** With the
tail regime isolated in phase C, S3 collapses the tail decoders:
...
tail faster. This is the **efficacy proof for the tail-waste half of the objective**,
with a t-statistic around −100 — decode-phase GPU occupancy is directly and
...
so its *relative* overhead shrinks as the batch grows. Its measured benefit — prefill
queueing/TTFT — should also grow with batch size, since the queue it drains is
proportional to the number of prompts arriving at the phase boundary.

### 7.7 Validation summary

| Mechanism | Result | Evidence strength |
|---|---|---|
| **S1 topology** | makespan −21.2% (2p2d vs 1p1d) | clean, t = −9.98 |
| **S2 role switch** | correct + lossless (100% valid) at **941 ms** per loaded flip; phase-A TTFT p50/p95 −31%…−39% vs same-suite static | quality strong; queue-timing benefit measured; wall clock transport-bound (not claimed) |
| **S3 consolidation** | **strongest**: tail decode-GPU·s −44.5%, avg decode-GPU −22% | clean, t ≈ −100 |
...
| **Method** | `business_wall == T_batch` (0 contamination); A/B wall sensitive to session order | — |

---

## Part 8 — Discussion and Future Work

**What is settled.** Consolidation is the clean win: it directly lowers decode-phase
GPU occupancy (−44.5% tail, t ≈ −100), which is exactly the intra-phase tail waste the
project set out to remove. Topology elasticity (−21%) is the stable substrate. Role
switch is proven correct and lossless; its wall-clock payoff is real in principle but
below the noise floor at this scale.
...
**Future work.** (1) Full 5-scenario interleaved suite with ≥5 repeats to resolve the
S2 wall-clock benefit. (2) Split-test the mixed fix (cordon-settle alone, `STABLE_SAMPLES`
back to 1–2) to recover the full GPU reclaim while keeping 100% valid. (3) Close the
loop: drive `switch_role` / `migrate` from a real GRPO rollout signal instead of a
harness. (4) Multi-node RDMA measurement. (5) Multi-engine support (SGLang, TRT-LLM).
