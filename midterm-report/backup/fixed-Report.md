...
1. Rollout-phase transitions. When the RL training job transitions between sampling (inference-heavy) and training (compute-heavy) phases, the entire inference cluster must spin up or wind down. The cluster must be pre-warmed before sampling begins and drained before training resumes.
2. Intra-phase load imbalance. Within a single rollout phase, the prefill/decode ratio may shift as the batch progresses — early in the phase prefill demand is high; later, decode dominates as long completions accumulate.
