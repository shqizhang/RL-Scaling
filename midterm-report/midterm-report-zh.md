...
**Prefill–Decode（PD）分离**是当前主流 LLM 推理架构（以 **NVIDIA Dynamo** 为代表），将计算受限的 prefill 与带宽受限的 decode 分池部署以最大化在线服务效率。然而在**强化学习（RL）后训练**场景中，推理流量随 rollout 阶段呈周期性阵发，两池交替空闲，GPU 工时严重浪费；水平扩缩容（冷启动 ≈ 数十秒）无法在单次 rollout 内介入。核心问题：**能否在数百毫秒内重塑 decode/prefill 池规模，且不丢失在飞请求、不打乱路由？**

本报告基于 Dynamo + vLLM 0.16 提出两个运行时原语：(1) **弹性 PD 角色切换**——状态机驱动的原地协议，通过 DWMD ModelCard 变更与 engine sleep/wake 循环实现亚秒级角色翻转，无需重建引擎或重部署 pod；(2) **在线解码请求合并**——三阶段块持有协议，经 NIXL RDMA 拉取源端 KV 块实现在飞请求跨 GPU 迁移，零 KV 丢失。配合 RL 信号驱动的自动伸缩控制器，二者使 decode/prefill 池可与 RL 阶段对齐地动态重塑。
